   ...
   <div class="col-9">
    		...
    		<div class="container">
			  <div class="row">
					...
					<tr>
						<td>
							...
							<c:if test="${sessionScope.userid ne null && (rpl.userid eq sessionScope.userid || sessionScope.userid eq 'admin')}">
								<button type="button" class="btn btn-md btn-outline-primary" onclick='location.href="/modBoard.board?boardNum=${board.boardNum}&category=${board.boardName}"'>수정</button>&nbsp;&nbsp;
								<button type="button" class="btn btn-md btn-outline-danger" onclick='location.href="/deleteBoard.board?boardNum=${board.boardNum}&category=${board.boardName}"'>삭제</button>
							</c:if>
						</td>
					</tr>
				</table>
				
				<form action="/regCommReply.board" method="post">
					<input type="hidden" value="${board.boardNum}" name="bno">
					<input type="hidden" value="${board.boardName}" name="category">
					<input type="hidden" value="${board.replyCnt}" name="replyCnt">
					<input type="hidden" value="${board.commCode}" name="commCode">
						  	...
						  	<c:forEach var="rpl" items="${replies}">
							      ...
							      <c:if test="${rpl.updateDate == null}">
							      	<td style="width: 20%; text-align: right;">
								      	...
								      	<c:if test="${rpl.userid == sessionScope.actName || sessionScope.userid == 'admin'}">
				...
				<form action="/modCommReply.board" method="post" name="modReplyFrm">
					<input type="hidden" name="modReplyNum" value="">
					<input type="hidden" name="modReplyBoardNum" value="${board.boardNum}">
					<input type="hidden" name="modReplyCategory" value="${board.boardName}">
					<input type="hidden" name="commCode" value="${board.commCode}">
				</form>
				
				<form action="/delCommReply.board" method="post" name="delReplyFrm">
					<input type="hidden" name="commCode" value="${board.commCode}">
				</form>
	...
	$(document).ready(function(){
		...
		$(".delRpl").on("click", function(){
			...
			if(confirm("댓글을 삭제하시겠습니까?")){
				...
				input2.setAttribute("value", '${board.boardName}');
